...
import { getDoc, setDoc, doc, updateDoc, onSnapshot, getDocs, collection, arrayUnion, arrayRemove } from "firebase/firestore";
...
const CustomeProductContext = ({ children }) => {
    ...
    const handleSearchBar = () => {
        const products = data;
        const filter = products.filter((item) => item.title.toLowerCase().includes(searchTerm.toLowerCase()));
        ...
        if (authenticated && user !== null) {
            ...
            if (!snapShot.empty) {
                ...
                if (docSnap.exists()) {
                    ...
                    const index = docSnap.data().list.findIndex((item) => item.prod.id === prod.id);
                    if (index === -1) {
    ...
    const handleRemoveCart = async (prod) => {
        if (authenticated && user !== null) {
            const uid = user.uid;
            const ref = collection(db, "usersCarts", uid, "myCart");
            const snapShot = await getDocs(ref);
    ...
    useEffect(() => {
        ...
        user && onSnapshot(doc(ref, "cart"), (doc) => {
    ...
    return (
        <productContext.Provider value={{ data, setData, cart, handleAddCart, handleRemoveCart, handleIncreaseQty, handleDecreaseQty,filteredData, searchTerm, setSearchTerm, handleSearchBar }}>
            {children}
